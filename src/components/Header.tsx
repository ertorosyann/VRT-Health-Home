...
const Header = () => {
  ...
  const navigationItems = [
    {
      name: navigation[currentLocale as keyof typeof navigation].home,
      href: '/',
    },
    {
      name: navigation[currentLocale as keyof typeof navigation].about,
      href: '/about',
    },
    {
      name: navigation[currentLocale as keyof typeof navigation].services,
      href: '/services',
    },
    {
      name: navigation[currentLocale as keyof typeof navigation].contact,
      href: '/contact',
    },
  ]
              ...
              {navigationItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`absolute bottom-0 left-1/2 h-0.5 bg-gradient-to-r from-transparent via-white to-transparent transition-all duration-300 ${
                        isActive
            ...
            <div className="hidden md:flex items-center space-x-2 lg:space-x-6">
              ...
              <div className="relative group">
                ...
                >
                  ...
                  <span className="relative z-10 text-xs sm:text-base md:text-xs lg:text-base font-title">
                    {
                      navigation[currentLocale as keyof typeof navigation]
                        .getStarted
                    }
                  </span>
                  <div className="relative z-10 w-4 h-4 sm:w-6 md:w-4 lg:w-6 bg-white/20 rounded-full flex items-center justify-center">
                    <svg
                      className="w-2 h-2 sm:w-4 md:w-2 lg:w-4 transform group-hover:translate-x-1 group-hover:scale-110 transition-all duration-300"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between p-4 sm:p-6 md:p-4 border-b border-white/20">
            ...
            >
              <svg
                className="w-6 h-6 sm:w-7 md:w-6 text-white"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              {navigationItems.map((item, index) => {
                ...
                return (
                  ...
                  >
                    {/* Background with gradient and blur effect */}
                    <div
                      className={`absolute inset-0 rounded-xl sm:rounded-2xl md:rounded-xl transition-all duration-300 ${
                        isActive
                          ? 'bg-gradient-to-r from-white/20 via-white/25 to-white/20 backdrop-blur-sm'
                          : 'bg-gradient-to-r from-white/5 via-white/10 to-white/5 backdrop-blur-sm opacity-0 group-hover:opacity-100'
                      }`}
                    ...
                    {/* Subtle border effect */}
                    <div
                      className={`absolute inset-0 rounded-xl sm:rounded-2xl md:rounded-xl border transition-all duration-300 ${
                        isActive
                          ? 'border-white/30'
                          : 'border-white/10 group-hover:border-white/25'
                      }`}
                    ...
                    <div
                      className={`absolute right-0 top-1/2 w-0.5 bg-gradient-to-b from-transparent via-white to-transparent transition-all duration-300 ${
                        isActive
                    ...
                    <div className="relative z-10 flex items-center justify-between">
                      <span
                        className={`text-lg sm:text-xl md:text-lg font-medium transition-colors duration-200 font-title ${
                          isActive
                      ...
                      <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-2">
                        ...
                        <svg
                          className={`w-5 h-5 sm:w-6 md:w-5 transition-all duration-200 ${
                            isActive
                              ? 'text-white'
